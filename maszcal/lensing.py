...
class SingleMassGnfwBaryonShear(GnfwBaryonShear):
    ...
    def delta_sigma_bary(self, rs, zs, mus, cons, alphas, betas, gammas):
        ...
        SHAPE z, r, params
        '''
        return (projector.esd(rs, lambda r: self.rho_bary(r, zs, mus, cons, alphas, betas, gammas))
...
class MatchingGnfwBaryonConvergence(Gnfw):
    ...
    def kappa_total(self, rs, zs, mus, cons, alphas, betas, gammas):
        return np.moveaxis(
            projector.sd(rs, lambda r: self.rho_tot(r, zs, mus, cons, alphas, betas, gammas)) * (u.Msun/u.Mpc**2).to(self.units),
            0,
            1,
...
class MatchingCmGnfwBaryonShear(CmGnfwBaryonShear):
    nfw_class: object = maszcal.nfw.MatchingCmNfwModel
...
@dataclass
class MatchingBaryonConvergenceModel(BaryonModel):
    convergence_class: object = MatchingGnfwBaryonConvergence
...
@dataclass
class MatchingBaryonShearModel(BaryonModel):
    shear_class: object = MatchingGnfwBaryonShear
